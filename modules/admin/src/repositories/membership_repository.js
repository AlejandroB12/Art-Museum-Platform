...
async function insert(userId, fecha, monto = 10.00) {
    return query("INSERT INTO Membresia (FechaPago, MontoPagado, id_usuario) VALUES (?, ?, ?)",
        [fecha, monto, userId]);
}
...
async function insertPaymentRequest(userId, monto = 10.00) {
    return query("INSERT INTO SolicitudPago (id_usuario, FechaSolicitud, Monto, Estatus) VALUES (?, NOW(), ?, 'Pendiente')",
        [userId, monto]);
}
